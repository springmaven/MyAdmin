...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            // 详细信息
            paySuccess: function(id,rePost ,width, height) {
                ...
                TOP.layer.open({
                    ...
                    shade: 0.3,
                    title: '支付结果',
                    content: '</br><font  style="margin-left: 150px;" size="4px"> 支付结果</font></br></br><font size="3px" style="margin-left: 20px">请在新的页面完成支付，支付完成后再关闭此页面</font> ',
                    zIndex: 9999999999,
                    btn: ['已完成支付', '支付遇到问题'],
                    // 弹层外区域关闭
                    shadeClose: true,
                    yes:function(index,layer){
                        ...
                        $.operate.post('/biz/order/pay',{'id':id});
                    },
            ...
            // 发货
            devilery: function(id,rePost,width,height) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.updateUrl.replace("{id}", id).replace("{rePost}",rePost);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
                ...
                $.modal.open(  $.table._option.modalName+"发货", url,width,height);
            },
            // 工具栏表格树修改
            editTree: function() {
